# Multi-Agent Retrieval System - Main Entry Point (Probabilistic Policy Version)
# This version uses probabilistic prediction model for decision making instead of agent policy

from model3 import MultiAgentRetrieval
...
def parse_args():
    ...
    parser.add_argument('--confidence_level', type=float, required=True, help='Confidence threshold for probabilistic policy. If next video matching probability is below this, reformulate.')
    parser.add_argument('--reformulation_type', type=str, default='normal',choices=['normal'])
...
def main(database_name,MLLM_model_id,search_model_name,featurename,rootpath,eval_k,examine_number,llama_device,vlm_device,search_device,server_device,BLIP2_server,ImageBind_server,BLIP2_feature_file,ImageBind_feature_file,MAX_ITER,confidence_level,start_query_idx,reformulation_type):
    # === Timing Setup ===
    timing_info = {}
    ...
    savepath_base = os.path.join("results_{eval_k}", 'Prob3_'+'MLLM_'+MLLM_model_id.split('/')[-2]+'_search_'+search_model_name+'_eval_k_'+str(eval_k)+'_examine_no_'+str(examine_number)+'_MAX_ITER_'+str(MAX_ITER)+'_confidence_'+str(confidence_level))
    ...
    try:
        for query_idx,(query_id,gt_video_name,query,query_key) in enumerate(queries):
            ...
            if not os.path.exists(savefile):
                ...
                wandb.init(project="Prob3_{search_model_name}_{eval_k}", name="Probabilistic3_"+database_name+"_"+MLLM_model_id.split('/')[-1]+"_"+search_model_name+"_"+str(eval_k)+'_examine_no_'+str(examine_number)+'_confidence_'+str(confidence_level)+'_query_'+str(query_id))
                ...
                try:
                    ...
                    ranklist,log_buffer, total_vlm_calls, vlm_verifications_at_pool_capacity = agent.run(query, max_steps=MAX_ITER,top_k=examine_number,log_file=log_file,gt=gt,fp=fp,unjudge_dict=unjudge_dict,reformulation_type=reformulation_type)
