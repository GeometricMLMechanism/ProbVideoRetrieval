...
class MultiAgentRetrieval:
    ...
    def vlm_score(self, query, rank_list,video_paths):
        ...
        for i, video_id in enumerate(rank_list):
            ...
            if video_id in self.verified_videos:
                print(f"  Video {i+1}/{len(rank_list)} ({video_id}): ⏭️  SKIPPED (already verified)")
            ...
            try:
                ...
                print(f"  Video {i+1}/{len(rank_list)} ({video_id}): {video_duration:.2f}s - {'✅ MATCH' if match_score else '❌ UNMATCH'} (VLM calls: {self.total_vlm_verifications})")
    ...
    def run(self, original_query, max_steps=3, top_k=5, log_file=None, gt=None, fp=None, unjudge_dict=None, reformulation_type='normal'):
        ...
        print(f"\n🚀 Starting ViSA Probabilistic Policy Run")
        ...
        for step in range(max_steps):
            ...
            print(f"🎯 Next Video Probability: {next_video_prob:.4f} (threshold: {self.confidence_level})")
        ...
        print(f"\n🏁 ViSA Probabilistic Policy Run Complete")
        print("=" * 80)
